class IdpBrowserSso():
    ...
    adapterMappings : array
        A list of adapters that map to incoming assertions.    artifact : str
        The settings for an artifact binding.    assertionsSigned : boolean
        Specify whether the incoming SAML assertions are signed rather than the entire SAML response being signed.    attributeContract : str
        The list of attributes that the IdP sends in the assertion.    authenticationPolicyContractMappings : array
        A list of Authentication Policy Contracts that map to incoming assertions.    authnContextMappings : array
        A list of authentication context mappings between local and remote values. Applicable for SAML 2.0 and OIDC protocol connections.    decryptionPolicy : str
        The SAML 2.0 decryption policy for browser-based SSO.    defaultTargetUrl : string
        The default target URL for this connection. If defined, this overrides the default URL.    enabledProfiles : str
        The profiles that are enabled for browser-based SSO. SAML 2.0 supports all profiles whereas SAML 1.x IdP connections support both IdP and SP (non-standard) initiated SSO. This is required for SAMLx.x Connections.    idpIdentityMapping : str
        Defines the process in which users authenticated by the IdP are associated with user accounts local to the SP.    incomingBindings : str
        ...
        The message customizations for browser-based SSO. Depending on server settings, connection type, and protocol this may or may not be supported.    oauthAuthenticationPolicyContractRef : str
        The Authentication policy contract to map into for OAuth. The policy contract can subsequently be mapped into the OAuth persistent grant.    oidcProviderSettings : str
        The OpenID Provider configuration settings. Required for an OIDC connection.    protocol : str
        The browser-based SSO protocol to use.    signAuthnRequests : boolean
        Determines whether SAML authentication requests should be signed.    sloServiceEndpoints : array
        A list of possible endpoints to send SLO requests and responses.    ssoOAuthMapping : str
        Direct mapping from the IdP connection to the OAuth persistent grant.    ssoServiceEndpoints : array
        The IdP SSO endpoints that define where to send your authentication requests. Only required for SP initiated SSO. This is required for SAML x.x and WS-FED Connections.    urlWhitelistEntries : array
