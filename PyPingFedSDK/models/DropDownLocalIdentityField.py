class DropDownLocalIdentityField():
    ...
    def _validate(self):
        return any(x for x in ['type', 'id', 'label', 'options'] if __dict__[x] is not None)
    ...
    def __hash__(self):
        return hash((attributes, defaultValue, id, label, options, profilePageField, registrationPageField, type))
