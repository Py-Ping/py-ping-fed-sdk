class DropDownLocalIdentityField():
 ...
 The type of the local identity field.
    ...
    def __hash__(self) -> int:
        return hash((self.attributes, self.defaultValue, self.var_id, self.label, self.options, self.profilePageField, self.registrationPageField, self.var_type))

    @classmethod
    def from_dict(cls, python_dict: dict):
        valid_data = {k: v for k, v in python_dict.items() if k in ["attributes", "defaultValue", "var_id", "label", "options", "profilePageField", "registrationPageField", "var_type"]}
