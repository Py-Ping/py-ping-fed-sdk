class ExportMetadataRequest():
    ...
    ----------
    connectionId : string
        The ID of the connection to export.    connectionType : str
        The type of connection to export.    signingSettings : str
        The signing settings to sign the metadata with. If null, the metadata will not be signed    useSecondaryPortForSoap : boolean
        If PingFederate's secondary SSL port is configured and you want to use it for the SOAP channel, set to true. If client-certificate authentication is configured for the SOAP channel, the secondary port is required and this must be set to true.    virtualHostName : string
        The virtual host name to be used as the base url.    virtualServerId : string
