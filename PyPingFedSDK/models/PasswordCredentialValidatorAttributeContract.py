class PasswordCredentialValidatorAttributeContract():
    ...
    coreAttributes : array
        A list of read-only attributes that are automatically populated by the password credential validator descriptor.    extendedAttributes : array
        A list of additional attributes that can be returned by the password credential validator. The extended attributes are only used if the adapter supports them.    inherited : boolean
