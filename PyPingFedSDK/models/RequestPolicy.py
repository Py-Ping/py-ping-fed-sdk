class RequestPolicy():
    ...
    allowUnsignedLoginHintToken : boolean
        Allow unsigned login hint token.    alternativeLoginHintTokenIssuers : array
        Alternative login hint token issuers.    authenticatorRef : str
        Reference to the associated authenticator.    id : string
        The request policy ID. ID is unique.    identityHintContract : str
        Identity hint attribute contract.    identityHintContractFulfillment : str
        Identity hint attribute contract fulfillment.    identityHintMapping : str
        Identity hint contract to request policy mapping.    name : string
        The request policy name. Name is unique.    requireTokenForIdentityHint : boolean
        Require token for identity hint.    transactionLifetime : integer
        The transaction lifetime in seconds.    userCodePcvRef : str
    ...
    def __hash__(self):
        return hash((self.allowUnsignedLoginHintToken, self.alternativeLoginHintTokenIssuers, self.authenticatorRef, self.id, self.identityHintContract, self.identityHintContractFulfillment, self.identityHintMapping, self.name, self.requireTokenForIdentityHint, self.transactionLifetime, self.userCodePcvRef))
