class IdpAdapter():
    """An IdP adapter instance.

    Attributes
    ----------
    attributeContract : str
        The list of attributes that the IdP adapter provides.    attributeMapping : str
        The attributes mapping from attribute sources to attribute targets.    authnCtxClassRef : string
        The fixed value that indicates how the user was authenticated.    configuration : str
    ...
    __slots__ = ["attributeContract", "attributeMapping", "authnCtxClassRef", "configuration", "id", "name", "parentRef", "pluginDescriptorRef"]
    ...
    def __eq__(self, other):
        if isinstance(other, IdpAdapter):
            return self.__dict__ == other.__dict__
        return NotImplemented

    def __hash__(self):
        return hash((self.attributeContract, self.attributeMapping, self.authnCtxClassRef, self.configuration, self.id, self.name, self.parentRef, self.pluginDescriptorRef))

    @classmethod
    def from_dict(cls, python_dict):
        valid_data = {k: v for k, v in python_dict.items() if k in ["attributeContract", "attributeMapping", "authnCtxClassRef", "configuration", "id", "name", "parentRef", "pluginDescriptorRef"]}
