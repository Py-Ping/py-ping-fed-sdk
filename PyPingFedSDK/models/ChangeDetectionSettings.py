...
TIMESTAMP_NO_NEGATION - Queries for timestamps on user records that are newer than the last time records were checked. This algorithm is recommended for the Oracle Directory Server.
    ...
    def _validate(self):
        return any(x for x in ['userObjectClass', 'groupObjectClass', 'changedUsersAlgorithm', 'timeStampAttributeName'] if __dict__[x] is not None)
    ...
    def __hash__(self):
        return hash((changedUsersAlgorithm, groupObjectClass, timeStampAttributeName, userObjectClass, usnAttributeName))
