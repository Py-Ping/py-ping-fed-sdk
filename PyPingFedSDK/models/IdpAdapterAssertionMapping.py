class IdpAdapterAssertionMapping():
    ...
    abortSsoTransactionAsFailSafe : boolean
        If set to true, SSO transaction will be aborted as a fail-safe when the data-store's attribute mappings fail to complete the attribute contract. Otherwise, the attribute contract with default values is used. By default, this value is false.    adapterOverrideSettings : str
        Connection specific configuration overrides for the mapped adapter instance.    attributeContractFulfillment : str
        A list of mappings from attribute names to their fulfillment values.    attributeSources : array
        A list of configured data stores to look up attributes from.    idpAdapterRef : str
        Reference to the associated IdP adapter.<br>Note: This is ignored if adapter overrides for this mapping exists. In this case, the override's parent adapter reference is used.    issuanceCriteria : str
