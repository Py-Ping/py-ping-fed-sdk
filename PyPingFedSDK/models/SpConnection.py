class SpConnection():
    ...
    active : boolean
        ...
        Additional allowed entities or issuers configuration. Currently only used in OIDC IdP (RP) connection.    applicationIconUrl : string
        The application icon url.    applicationName : string
        The application name.    attributeQuery : str
        The attribute query settings for supporting SPs in requesting user attributes.    baseUrl : string
        ...
        The connection name.    outboundProvision : str
        The Outbound Provision settings.    spBrowserSso : str
        The browser-based SSO settings used to communicate with your SP.    type : str
        The type of this connection. This must be set to 'SP'.    virtualEntityIds : array
    ...
    def __hash__(self):
        return hash((self.active, self.additionalAllowedEntitiesConfiguration, self.applicationIconUrl, self.applicationName, self.attributeQuery, self.baseUrl, self.contactInfo, self.credentials, self.defaultVirtualEntityId, self.entityId, self.extendedProperties, self.id, self.licenseConnectionGroup, self.loggingMode, self.metadataReloadSettings, self.name, self.outboundProvision, self.spBrowserSso, self.type, self.virtualEntityIds, self.wsTrust))
