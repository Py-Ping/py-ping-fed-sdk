class DynamicClientRegistration():
    ...
    allowedExclusiveScopes : array
        The exclusive scopes to allow.    bypassActivationCodeConfirmationOverride : boolean
        Indicates if the Activation Code Confirmation page should be bypassed if 'verification_url_complete' is used by the end user to authorize a device.    cibaPollingInterval : integer
        The minimum amount of time in seconds that the Client must wait between polling requests to the token endpoint. The default is 3 seconds.    cibaRequireSignedRequests : boolean
        Determines whether CIBA signed requests are required for this client.    clientCertIssuerRef : str
        Client TLS Certificate Issuer DN.    clientCertIssuerType : str
        Client TLS Certificate Issuer Type.    defaultAccessTokenManagerRef : str
        The default access token manager for this client.    deviceFlowSettingType : str
        ...
        The amount of time client should wait between polling requests, in seconds.    enforceReplayPrevention : boolean
        Enforce replay prevention.    initialAccessTokenScope : string
        The initial access token to prevent unwanted client registrations.    oidcPolicy : str
        ...
        The 'device_code' and 'user_code' timeout, in seconds.    persistentGrantExpirationTime : integer
        The persistent grant expiration time.    persistentGrantExpirationTimeUnit : str
        ...
        Allows an administrator to override the Persistent Grant Idle Timeout set globally for the OAuth AS. Defaults to SERVER_DEFAULT.    policyRefs : array
        The client registration policies.    refreshRolling : str
        Use ROLL or DONT_ROLL to override the Roll Refresh Token Values setting on the Authorization Server Settings. SERVER_DEFAULT will default to the Roll Refresh Token Values setting on the Authorization Server Setting screen. Defaults to SERVER_DEFAULT.    requestPolicyRef : str
        The CIBA request policy.    requireProofKeyForCodeExchange : boolean
        Determines whether Proof Key for Code Exchange (PKCE) is required for the dynamically created client.    requireSignedRequests : boolean
        Require signed requests.    restrictCommonScopes : boolean
        Restrict common scopes.    restrictedCommonScopes : array
        The common scopes to restrict.    tokenExchangeProcessorPolicyRef : str
