class CustomDataStore():
    ...
    configuration : str
        ...
        Whether attribute values should be masked in the log.    name : string
        The plugin instance name.    parentRef : str
        ...
        Reference to the plugin descriptor for this instance. The plugin descriptor cannot be modified once the instance is created.<br>Note: Ignored when specifying a connection's adapter override.    type : str
        The data store type.
    """
    ...
    def __hash__(self):
        return hash((self.configuration, self.id, self.maskAttributeValues, self.name, self.parentRef, self.pluginDescriptorRef, self.type))
