class AccessTokenAttributeContract():
    ...
    coreAttributes : array
        A list of core token attributes that are associated with the access token management plugin type. This field is read-only and is ignored on POST/PUT.    defaultSubjectAttribute : string
        Default subject attribute to use for audit logging when validating the access token. Blank value means to use USER_KEY attribute value after grant lookup.    extendedAttributes : array
        A list of additional token attributes that are associated with this access token management plugin instance.    inherited : boolean
