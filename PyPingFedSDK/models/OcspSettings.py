class OcspSettings():
    ...
    actionOnResponderUnavailable : string
        Action on responder unavailable. This value defaults to  "CONTINUE".    actionOnStatusUnknown : string
        Action on status unknown. This value defaults to  "FAIL".    actionOnUnsuccessfulResponse : string
        Action on unsuccessful response. This value defaults to  "FAIL".    currentUpdateGracePeriod : integer
        Current update grace period in minutes. This value defaults to "5".    nextUpdateGracePeriod : integer
        Next update grace period in minutes. This value defaults to "5".    requesterAddNonce : boolean
        Do not allow responder to use cached responses. This setting defaults to disabled.    responderCertReference : str
        Resource link to OCSP responder signature verification certificate. A previously selected certificate will be deselected if this attribute is not defined.    responderTimeout : integer
        Responder connection timeout in seconds. This value defaults to "5".    responderUrl : string
        Responder URL address. This field is required if OCSP revocation is enabled.    responseCachePeriod : integer
