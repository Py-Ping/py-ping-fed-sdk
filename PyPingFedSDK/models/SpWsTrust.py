class SpWsTrust():
    ...
    abortIfNotFulfilledFromRequest : boolean
        If the attribute contract cannot be fulfilled using data from the Request, abort the transaction.    attributeContract : str
        A set of user attributes that the IdP sends in the token.    defaultTokenType : str
        The default token type when a web service client (WSC) does not specify in the token request which token type the STS should issue. Defaults to SAML 2.0.    encryptSaml2Assertion : boolean
        When selected, the STS encrypts the SAML 2.0 assertion. Applicable only to SAML 2.0 security token.  This option does not apply to OAuth assertion profiles.    generateKey : boolean
        When selected, the STS generates a symmetric key to be used in conjunction with the "Holder of Key" (HoK) designation for the assertion's Subject Confirmation Method.  This option does not apply to OAuth assertion profiles.    messageCustomizations : array
        The message customizations for WS-Trust. Depending on server settings, connection type, and protocol this may or may not be supported.    minutesAfter : integer
        The amount of time after the SAML token was issued during which it is to be considered valid. The default value is 30.    minutesBefore : integer
        The amount of time before the SAML token was issued during which it is to be considered valid. The default value is 5.    oAuthAssertionProfiles : boolean
        When selected, four additional token-type requests become available.    partnerServiceIds : array
        The partner service identifiers.    requestContractRef : str
        Request Contract to be used to map attribute values into the security token.    tokenProcessorMappings : array
