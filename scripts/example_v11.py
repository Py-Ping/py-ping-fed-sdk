...
with Container(home, ping_user, ping_key, version="11.0.0-edge") as container:
    ...
    idp_sp_conn = IdpSpConnections(endpoint, session).getConnectionCerts_1(
        id='OTIdPJava'
    ).to_dict()
    ...
    pprint(AuthenticationApi(endpoint, session).getAuthenticationApiSettings())
