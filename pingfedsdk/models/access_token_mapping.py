...
class AccessTokenMapping(Model):
    ...
    """
    def __init__(self, id: str, context: AccessTokenMappingContext, accessTokenManagerRef: ResourceLink, attributeContractFulfillment: dict, attributeSources: list = None, issuanceCriteria: IssuanceCriteria = None) -> None:
        self.id = id
    ...
    def _validate(self) -> bool:
        return any(x for x in ["accessTokenManagerRef", "attributeContractFulfillment", "context", "id"] if self.__dict__[x] is not None)
    ...
    def __hash__(self) -> int:
        return hash(frozenset([self.id, self.context, self.accessTokenManagerRef, self.attributeSources, self.attributeContractFulfillment, self.issuanceCriteria]))

    @classmethod
    def from_dict(cls, python_dict: dict):
        valid_data = {}
        for k, v in python_dict.items():
            if k in ["id", "context", "accessTokenManagerRef", "attributeSources", "attributeContractFulfillment", "issuanceCriteria"] and v is not None:
                if k == "id":
                    valid_data[k] = str(v)
                if k == "context":
    ...
    def to_dict(self, remove_nonetypes=False):
        ...
        for k, v in self.__dict__.items():
            if k in ["id", "context", "accessTokenManagerRef", "attributeSources", "attributeContractFulfillment", "issuanceCriteria"]:
