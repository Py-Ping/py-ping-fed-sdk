...
class CertView(Model):
    ...
    def __hash__(self) -> int:
        return hash(frozenset([self.id, self.serialNumber, self.subjectDN, self.subjectAlternativeNames, self.issuerDN, self.validFrom, self.expires, self.keyAlgorithm, self.keySize, self.signatureAlgorithm, self.version, self.sha1Fingerprint, self.sha256Fingerprint, self.status, self.cryptoProvider]))

    @classmethod
    def from_dict(cls, python_dict: dict):
        valid_data = {}
        for k, v in python_dict.items():
            if k in ["id", "serialNumber", "subjectDN", "subjectAlternativeNames", "issuerDN", "validFrom", "expires", "keyAlgorithm", "keySize", "signatureAlgorithm", "version", "sha1Fingerprint", "sha256Fingerprint", "status", "cryptoProvider"] and v is not None:
                if k == "id":
                    valid_data[k] = str(v)
                if k == "serialNumber":
                    valid_data[k] = str(v)
    ...
    def to_dict(self, remove_nonetypes=False):
        ...
        for k, v in self.__dict__.items():
            if k in ["id", "serialNumber", "subjectDN", "subjectAlternativeNames", "issuerDN", "validFrom", "expires", "keyAlgorithm", "keySize", "signatureAlgorithm", "version", "sha1Fingerprint", "sha256Fingerprint", "status", "cryptoProvider"]:
