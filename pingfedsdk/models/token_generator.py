...
from pingfedsdk.models.token_generator_attribute_contract import TokenGeneratorAttributeContract


class TokenGenerator(Model):
    ...
    attributeContract: TokenGeneratorAttributeContract
        The list of attributes that the token generator provides.

    """
    def __init__(self, id: str, name: str, pluginDescriptorRef: ResourceLink, configuration: PluginConfiguration, parentRef: ResourceLink = None, attributeContract: TokenGeneratorAttributeContract = None) -> None:
        self.id = id
        self.name = name
    ...
    def __hash__(self) -> int:
        return hash(frozenset([self.id, self.name, self.pluginDescriptorRef, self.parentRef, self.configuration, self.attributeContract]))

    @classmethod
    def from_dict(cls, python_dict: dict):
        valid_data = {}
        for k, v in python_dict.items():
            if k in ["id", "name", "pluginDescriptorRef", "parentRef", "configuration", "attributeContract"] and v is not None:
                if k == "id":
                    valid_data[k] = str(v)
                if k == "name":
                    valid_data[k] = str(v)
                ...
                if k == "attributeContract":
                    valid_data[k] = TokenGeneratorAttributeContract.from_dict(v)
    ...
    def to_dict(self, remove_nonetypes=False):
        ...
        for k, v in self.__dict__.items():
            if k in ["id", "name", "pluginDescriptorRef", "parentRef", "configuration", "attributeContract"]:
