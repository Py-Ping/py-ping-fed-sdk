from enum import Enum

from pingfedsdk.model import Model


class SpAttributeQueryPolicy(Model):
    """The attribute query profile's security policy.

    Attributes
    ----------
    signResponse: bool
        Sign the response.

    signAssertion: bool
        Sign the assertion.

    encryptAssertion: bool
        Encrypt the assertion.

    requireSignedAttributeQuery: bool
        Require signed attribute query.

    requireEncryptedNameId: bool
        Require an encrypted name identifier.

    """
    def __init__(self, signResponse: bool = None, signAssertion: bool = None, encryptAssertion: bool = None, requireSignedAttributeQuery: bool = None, requireEncryptedNameId: bool = None) -> None:
        self.signResponse = signResponse
        self.signAssertion = signAssertion
        self.encryptAssertion = encryptAssertion
        self.requireSignedAttributeQuery = requireSignedAttributeQuery
        self.requireEncryptedNameId = requireEncryptedNameId

    def _validate(self) -> bool:
        return any(x for x in [] if self.__dict__[x] is not None)

    def __eq__(self, other) -> bool:
        if isinstance(other, SpAttributeQueryPolicy):
            return self.__dict__ == other.__dict__
        return NotImplemented

    def __hash__(self) -> int:
        return hash(frozenset([self.signResponse, self.signAssertion, self.encryptAssertion, self.requireSignedAttributeQuery, self.requireEncryptedNameId]))

    @classmethod
    def from_dict(cls, python_dict: dict):
        valid_data = {}
        for k, v in python_dict.items():
            if k in ["signResponse", "signAssertion", "encryptAssertion", "requireSignedAttributeQuery", "requireEncryptedNameId"] and v is not None:
                if k == "signResponse":
                    valid_data[k] = bool(v)
                if k == "signAssertion":
                    valid_data[k] = bool(v)
                if k == "encryptAssertion":
                    valid_data[k] = bool(v)
                if k == "requireSignedAttributeQuery":
                    valid_data[k] = bool(v)
                if k == "requireEncryptedNameId":
                    valid_data[k] = bool(v)

        return cls(**valid_data)

    def to_dict(self, remove_nonetypes=False):
        """
        Naive dictionary serialiser. Recursively handles most types in this
        module. If you make your own class you need to inherit from model
        and implement to_dict.
        """
        body = {}
        for k, v in self.__dict__.items():
            if k in ["signResponse", "signAssertion", "encryptAssertion", "requireSignedAttributeQuery", "requireEncryptedNameId"]:
                if isinstance(v, Model):
                    body[k] = v.to_dict(remove_nonetypes)
                elif isinstance(v, list):
                    vals = []
                    for x in v:
                        if isinstance(x, Model):
                            vals.append(x.to_dict(remove_nonetypes))
                        elif not remove_nonetypes or (remove_nonetypes and x is not None):
                            vals.append(x)
                    body[k] = vals
                elif isinstance(v, dict):
                    vals = {}
                    for x, y in v.items():
                        if isinstance(y, Model):
                            vals[x] = y.to_dict(remove_nonetypes)
                        elif not remove_nonetypes or (remove_nonetypes and y is not None):
                            vals[x] = y
                    body[k] = vals
                elif isinstance(v, set):
                    vals = []
                    for x in v:
                        if isinstance(x, Model):
                            vals.append(x.to_dict(remove_nonetypes))
                        elif not remove_nonetypes or (remove_nonetypes and x is not None):
                            vals.append(x)
                    body[k] = vals
                elif isinstance(v, Enum):
                    body[k] = str(v).split('.')[-1]
                elif not remove_nonetypes or (remove_nonetypes and v is not None):
                    body[k] = v
        return body
