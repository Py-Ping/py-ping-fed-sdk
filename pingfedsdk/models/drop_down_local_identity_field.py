...
class DropDownLocalIdentityField(Model):
    ...
    def _validate(self) -> bool:
        return any(x for x in ["options"] if self.__dict__[x] is not None)
    ...
    def __hash__(self) -> int:
        return hash(frozenset([self.type, self.id, self.label, self.registrationPageField, self.profilePageField, self.attributes, self.options, self.defaultValue]))
    ...
    def to_dict(self, remove_nonetypes=False):
        ...
        for k, v in self.__dict__.items():
            if k in ["type", "id", "label", "registrationPageField", "profilePageField", "attributes", "options", "defaultValue"]:
