...
class BaseSigningSettings(Model):
    """Settings related to signing messages.
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, BaseSigningSettings):
            return self.__dict__ == other.__dict__
        return NotImplemented

    def __hash__(self) -> int:
        return hash(frozenset([self.signingKeyPairRef, self.algorithm, self.includeCertInSignature, self.includeRawKeyInSignature]))
    ...
    def to_dict(self, remove_nonetypes=False):
        ...
        for k, v in self.__dict__.items():
            if k in ["signingKeyPairRef", "algorithm", "includeCertInSignature", "includeRawKeyInSignature"]:
