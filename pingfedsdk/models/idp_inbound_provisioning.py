...
class IdpInboundProvisioning(Model):
    ...
    """
    def __init__(self, groupSupport: bool, userRepository: InboundProvisioningUserRepository, customSchema: Schema, users: Users, groups: Groups, actionOnDelete: ActionOnDelete = None) -> None:
        ...
        for k, v in python_dict.items():
            if k in ["groupSupport", "userRepository", "customSchema", "users", "groups", "actionOnDelete"] and v is not None:
                if k == "groupSupport":
                    valid_data[k] = bool(v)
                if k == "userRepository":
                    valid_data[k] = InboundProvisioningUserRepository.from_dict(v)
                if k == "customSchema":
                    valid_data[k] = Schema.from_dict(v)
                if k == "users":
                    valid_data[k] = Users.from_dict(v)
                if k == "groups":
                    valid_data[k] = Groups.from_dict(v)
                if k == "actionOnDelete":
