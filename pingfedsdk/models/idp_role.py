...
class IdpRole(Model):
    ...
    saml20Profile: SAML20Profile
        SAML 2.0 Profile settings.

    enableOutboundProvisioning: bool
    ...
    def __hash__(self) -> int:
        return hash(frozenset([self.enable, self.enableSaml11, self.enableSaml10, self.enableWsFed, self.enableWsTrust, self.saml20Profile, self.enableOutboundProvisioning]))

    @classmethod
    def from_dict(cls, python_dict: dict):
        valid_data = {}
        for k, v in python_dict.items():
            if k in ["enable", "enableSaml11", "enableSaml10", "enableWsFed", "enableWsTrust", "saml20Profile", "enableOutboundProvisioning"] and v is not None:
                if k == "enable":
                    valid_data[k] = bool(v)
                if k == "enableSaml11":
                    valid_data[k] = bool(v)
    ...
    def to_dict(self, remove_nonetypes=False):
        ...
        for k, v in self.__dict__.items():
            if k in ["enable", "enableSaml11", "enableSaml10", "enableWsFed", "enableWsTrust", "saml20Profile", "enableOutboundProvisioning"]:
